...
import { fetchStartups } from "../lib/queries"
...
export default function HomePage() {
  ...
  const activeCompanies = startups.filter(s => s.status === 'Active').length
  const totalCategories = categories.length
  const avgTeamSize = Math.round(
    startups.reduce((sum, s) => sum + (s.team_size || 0), 0) / (startups.length || 1)
  )
      ...
      <section className="py-16 px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 m-15">
          {filteredStartups.map((startup, i) => (
            <StartupCard key={startup._id || i} startup={startup} />
          ))}
        </div>
      </section>
    </div>
